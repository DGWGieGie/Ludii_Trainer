...
class Model:
    ...
    def create_model(self):
        ...
        self.input = tf.compat.v1.placeholder(shape=[None, self.num_inputs], dtype=tf.float32)
        cur = self.input
        size = self.num_inputs
        while size > MIN_PRE_SIZE:
            size = max(MIN_PRE_SIZE, size // 2)
            # cur = dense(cur, size, activation=tf.nn.relu)
            cur = dense(size, activation=tf.nn.relu)(cur)
        # cur = dense(cur, size, activation=tf.nn.relu)
        ...
        for role in self.roles:
            ...
            logits = dense(final, activation=None)(cur)
            probs = tf.nn.softmax(logits)
            #q = dense(cur, 1, activation=tf.nn.sigmoid)
            q = dense(1, activation=tf.nn.sigmoid)(cur)
    ...
    def create_trainer(self):
        ...
        #                for role in self.roles}
        self.target = {role: (tf.compat.v1.placeholder(tf.float32, shape=(None, 1)),
                          tf.compat.v1.placeholder(tf.float32, shape=(None, self.num_actions[role])))
                   for role in self.roles}
        ...
        for role, (q, logits, probs) in self.outputs.items():
            ...
            self.loss += tf.reduce_mean(tf.compat.v1.losses.softmax_cross_entropy(onehot_labels=tprobs, logits=logits))
        ...
        self.optimiser = tf.compat.v1.train.AdamOptimizer(0.01)
